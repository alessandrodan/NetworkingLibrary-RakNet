...
public:
	...
	{
		return readOffset > numberOfBitsUsed ? 0 : numberOfBitsUsed - readOffset;
	}
	...
	{
		bool r = IsNetworkOrderInternal(); return r;
	}
...
{
	static_assert(std::is_trivially_copyable_v<templateType>);

#ifdef _MSC_VER
#pragma warning(disable:4127)   // conditional expression is constant
#endif
	if(sizeof(inTemplateVar) == 1)
		WriteBits((unsigned char*)&inTemplateVar, sizeof(inTemplateVar) * 8, true);
	else
	{
#ifndef __BITSTREAM_NATIVE_END
		if(DoEndianSwap())
		{
			unsigned char output[sizeof(inTemplateVar)];
			ReverseBytes((unsigned char*)&inTemplateVar, output, sizeof(inTemplateVar));
			WriteBits((unsigned char*)output, sizeof(output) * 8, true);
		}
		else
#endif
			WriteBits((unsigned char*)&inTemplateVar, sizeof(inTemplateVar) * 8, true);
...
inline void BitStream::Write(const SystemAddress &inTemplateVar)
{
	...
	if(inTemplateVar.GetIPVersion() == 4)
	{
		...
		uint16_t port = inTemplateVar.GetPortNetworkOrder();
		WriteBits((unsigned char*)&port, sizeof(port) * 8, true);
	}
	else
	{
		// Don't endian swap
...
#pragma warning(disable:4127)   // conditional expression is constant
	...
	if(sizeof(outTemplateVar) == 1)
		return ReadBits((unsigned char*)&outTemplateVar, sizeof(outTemplateVar) * 8, true);
	else
	{
#ifndef __BITSTREAM_NATIVE_END
		if(DoEndianSwap())
		{
			unsigned char output[sizeof(outTemplateVar)];
			if(ReadBits((unsigned char*)output, sizeof(output) * 8, true))
			{
				ReverseBytes(output, (unsigned char*)&outTemplateVar, sizeof(outTemplateVar));
				return true;
			}
			return false;
		}
...
inline bool BitStream::Read(SystemAddress &outTemplateVar)
{
	...
	if(ipVersion == 4)
	{
		...
		return ReadBits((unsigned char*)&outTemplateVar.address.addr4.sin_port, sizeof(outTemplateVar.address.addr4.sin_port) * 8, true);
	}
	else
	{
		return ReadBits((unsigned char*)&outTemplateVar.address.addr6, sizeof(outTemplateVar.address.addr6) * 8, true);
	}
}

...
public:
	...
	static void DomainNameToIP(const char *domainName, char ip[INET6_ADDRSTRLEN]);
...
struct RNS2_BerkleyBindParameters
{
	...
	unsigned short port;
	char *hostAddress;
	unsigned short addressFamily; // AF_UNSPEC, AF_INET or AF_INET6
	int type; // SOCK_DGRAM
	int protocol; // 0
...
protected:
	...
	static void GetSystemAddress(RNS2Socket rns2Socket, SystemAddress *systemAddressOut);

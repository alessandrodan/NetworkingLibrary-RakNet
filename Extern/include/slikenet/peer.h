...
{
	unsigned index; RemoteSystemIndex *next;
};
...
public:
	...
	/// \param[in] host Either an IP address or a domain name.
	...
	/// \details The remote system, if it is Initialized, will respond with ID_UNCONNECTED_PONG followed by sizeof(SLNet::TimeMS) containing the system time the ping was sent. Default is 4 bytes - See __GET_TIME_64BIT in types.h
	/// System should reply with ID_UNCONNECTED_PONG if it is active
	...
	/// By default this is false. Recommended to activate it, because congestion control uses it to determine how often to resend lost packets.
...
//		int connectionSocketIndex; // index into connectionSockets to send back on.
		...
		enum ConnectMode
		{
			NO_ACTION, DISCONNECT_ASAP, DISCONNECT_ASAP_SILENTLY, DISCONNECT_ON_NO_ACK, REQUESTED_CONNECTION, HANDLING_CONNECTION_REQUEST, UNVERIFIED_SENDER, CONNECTED
		} connectMode;
	};
...
protected:
	...
	struct BufferedCommandStruct
	{
		...
		enum
		{
			BCS_SEND, BCS_CLOSE_CONNECTION, BCS_GET_SOCKET, BCS_CHANGE_SYSTEM_ADDRESS,/* BCS_USE_USER_SOCKET, BCS_REBIND_SOCKET_ADDRESS, BCS_RPC, BCS_RPC_SHIFT,*/ BCS_DO_NOTHING
		} command;
	};

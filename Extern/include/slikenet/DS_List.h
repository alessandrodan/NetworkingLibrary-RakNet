...
List<list_type>::~List()
{
	if(allocation_size > 0)
		SLNet::OP_DELETE_ARRAY(list_array, _FILE_AND_LINE_);
}
...
List<list_type>::List(const List& original_copy)
{
	...
	{
		list_array = SLNet::OP_NEW_ARRAY<list_type>(original_copy.list_size, _FILE_AND_LINE_);

		for(unsigned int counter = 0; counter < original_copy.list_size; ++counter)
			list_array[counter] = original_copy.list_array[counter];
...
void List<list_type>::Insert(const list_type &input, const unsigned int position, const char *file, unsigned int line)
{
	...
	if(list_size == allocation_size)
	{
		...
		for(unsigned int counter = 0; counter < list_size; ++counter)
			new_array[counter] = list_array[counter];
		...
		SLNet::OP_DELETE_ARRAY(list_array, file, line);
	...
	for(unsigned int counter = list_size; counter != position; counter--)
		list_array[counter] = list_array[counter - 1];
...
#endif
	--list_size;
	list_array[position] = list_array[list_size];
}

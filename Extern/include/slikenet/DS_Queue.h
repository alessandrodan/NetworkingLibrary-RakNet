...
public:
	...
	void ClearAndForceAllocation(unsigned int size, const char *file, unsigned int line); // Force a memory allocation to a certain larger size
...
void Queue<queue_type>::Push(const queue_type& input, const char *file, unsigned int line)
{
	...
	if(tail == head)
	{
		...
		new_array = SLNet::OP_NEW_ARRAY<queue_type>(allocation_size * 2, file, line);
#ifdef _DEBUG
		RakAssert(new_array);
#endif
		if(new_array == 0)
...
template <class queue_type>
void Queue<queue_type>::ClearAndForceAllocation(unsigned int size, const char *file, unsigned int line)
{
	SLNet::OP_DELETE_ARRAY(array, file, line);
	if(size > 0)
